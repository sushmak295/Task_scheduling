...
'''dynamic=[{'name':'w1','period':520,0:53,1:66,2:89,3:141},
         {'name':'w2','period':220,0:40,1:50,2:67,3:114},
         {'name':'w3','period':500,0:104,1:134,2:184,3:313},
         {'name':'w4','period':200,0:57,1:74,2:103,3:175},
         {'name':'w5','period':300,0:35,1:62,2:62,3:104,}]"'''
...
for i in range(0,4):
    for j in range(0,4):
        for k in range(0,4):
            for l in range(0,4):
                for m in range(0,4):
                    e=(dynamic[0][i]/float(dynamic[0]['period']))+(dynamic[1][j]/float(dynamic[1]['period']))+(dynamic[2][k]/float(dynamic[2]['period']))+(dynamic[3][l]/float(dynamic[3]['period']))+(dynamic[4][m]/float(dynamic[4]['period']))
                    b=b+1
                    if e<=5*(2**.2-1):
                        #print "w1 at:",frequency[i]
                        #print "w2 at:",frequency[j]
                        #print "w3 at:",frequency[k]
                        #print "w4 at:",frequency[l]
                        #print "w5 at:",frequency[m]
                        totener=(dynamic[0][i]*(energy[i]))+(dynamic[1][j]*(energy[j]))+(dynamic[2][k]*(energy[k]))+(dynamic[3][l]*(energy[l]))+(dynamic[4][m]*(energy[m]))
                        #print "total energy:",totener
...
if len(combination)!=0:
    print "best combination:",combination[0]
    ...
    def RM(i):
        ...
        if len(lists)!=0:
            lists.sort(key=operator.itemgetter('period'))
            a.append(lists[0])
    ...
    if lrm<=rrm:
        ...
        print "start task end  frequrncy  energy"            
