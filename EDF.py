...
for j in range(1,length):
    ...
    lrm=lrm+tasks[j]['exetym']/float(tasks[j]['period'])#+tasks[2]['exetym']/float(tasks[2]['period'])+tasks[3]['exetym']/float(tasks[3]['period'])+tasks[4]['exetym']/float(tasks[4]['period'])+tasks[5]['exetym']/float(tasks[5]['period'])   
...
if lrm<=rrm:
    ...
    print "EDF scheduling can not be acheived"
